...
abstract class BeaingPreparedOrdersController extends GetxController {
  ...
  List settings = [];
  String? lang;
  List beaingPreparedOrders = [];
  TextEditingController searchController = TextEditingController();
  bool? isSearch;
  List<ItemsModel> itemsSearchList = [];
  initialData();
  getSettings();
  getBeaingPreparedOrders(int userId);
}

class BeaingPreparedOrdersControllerImp extends BeaingPreparedOrdersController {
  ...
  initialData() async {
    ...
    getBeaingPreparedOrders(userId!);
  }
  ...
  @override
  getBeaingPreparedOrders(int userId) async {
    statusRequest = StatusRequest.loading;
    update();
    beaingPreparedOrders.clear();
    try {
      statusRequest = StatusRequest.loading;
      var response = await orderData.getBeaingPreparedOrders();
      print("============================== $response");
      statusRequest = handlingData(response);
      if (StatusRequest.success == statusRequest) {
        if (response['status'] == "success") {
          beaingPreparedOrders.addAll(response['orders']);
        } else {
